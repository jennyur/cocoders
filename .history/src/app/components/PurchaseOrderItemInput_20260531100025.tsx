...
const UNIT_OPTIONS = ["kg", "g", "pcs", "liters", "bottles", "packs", "boxes", "dozen"];
...
export function PurchaseOrderItemInput({
  ...
  const matchingProducts = useMemo(() => {
    ...
    return productDatabase.filter((product) => product.name.toLowerCase().includes(normalizedQuery));
  }, [normalizedQuery, productDatabase]);

  const exactMatch = useMemo(
    () => productDatabase.find((product) => product.name.toLowerCase() === normalizedQuery),
    [normalizedQuery, productDatabase]
  );
  ...
  const canAddItem = Boolean(
    value.productName.trim() &&
      ...
      (!value.isNewProduct || (value.category.trim() && value.unit.trim()))
  );
